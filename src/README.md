...
Cirtech CP/M boot disk, creating `.BIN` binary image files plus `CPM3.SYS`. It then builds the assembler files and links them into `.BIN` files and `BNKBIOS3.SPR` file for `GENCPM.COM`. 

Gencpm is run to generate `CPM3.SYS` and the result place into the `binaries` folder.

Finally it compares the files from the `binaries` folder generated by the NibReader and stored in the `comparison` folder. 
